...
export async function appendAdviceDigest(opts: {
  ...
  const res = await fetch(ANTHROPIC_URL, {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 150,
      temperature: 0.0,
      system: "Extract the single most important legal conclusion from this AI legal advice. " +
...
export function formatAdviceDigestBlock(entries: AdviceDigestEntry[]): string {
  ...
  const lines = ["<prior_advice>", "Previous conclusions from earlier sessions:"];
  for (const e of recent) {
    lines.push(`  [${e.session_date}] ${e.conclusion}${e.probability_signal ? ` (${e.probability_signal})` : ""}`);
  }
  lines.push("If your current answer contradicts a prior conclusion, explicitly acknowledge the change and explain why.");
