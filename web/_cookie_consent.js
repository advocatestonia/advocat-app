...
(function () {
  ...
  var TEXTS = {
    en: {
      ...
      analytics_desc:
        "Aggregated usage analytics (GA4) to improve the service.",
    ...
    fr: {
      ...
      necessary_desc:
        "Requis pour l'authentification et la sécurité. Toujours actifs.",
      analytics_desc:
        "Mesure d'audience agrégée (GA4) pour améliorer le service.",
      marketing_desc:
        "Suivi des conversions sur les plateformes publicitaires.",
      functional_desc:
        "Fonctionnalités optionnelles (mémorisation des préférences).",
    ...
    es: {
      ...
      necessary_desc:
        "Necesarias para la autenticación y la seguridad. Siempre activas.",
      analytics_desc:
        "Analítica de uso agregada (GA4) para mejorar el servicio.",
      marketing_desc:
        "Seguimiento de conversiones en plataformas publicitarias.",
      ...
      migrate_msg:
        "Preferencias de cookies actualizadas. Revisa tu configuración.",
    ...
    it: {
      ...
      necessary_desc:
        "Necessari per autenticazione e sicurezza. Sempre attivi.",
      analytics_desc:
        "Analitica d'uso aggregata (GA4) per migliorare il servizio.",
    ...
    de: {
      ...
      analytics_desc:
        "Aggregierte Nutzungsanalyse (GA4) zur Verbesserung des Dienstes.",
    ...
    nl: {
      ...
      analytics_desc:
        "Geaggregeerde gebruiksanalyse (GA4) om de dienst te verbeteren.",
      marketing_desc: "Conversietracking op advertentieplatformen.",
      functional_desc:
        "Optionele UX-functies (bijv. onthouden van voorkeuren).",
    ...
    pt: {
      ...
      necessary_desc:
        "Necessários para autenticação e segurança. Sempre ativos.",
      analytics_desc:
        "Análise de utilização agregada (GA4) para melhorar o serviço.",
      marketing_desc:
        "Acompanhamento de conversões em plataformas publicitárias.",
      ...
      migrate_msg:
        "Preferências de cookies atualizadas. Reveja as suas definições.",
      ...
      necessary_desc:
        "Tarvitaan kirjautumiseen ja turvallisuuteen. Aina päällä.",
      analytics_desc:
        "Anonyymi käyttöanalytiikka (GA4) palvelun parantamiseksi.",
      marketing_desc: "Mainontaplatformien konversioseuranta.",
      functional_desc:
        "Valinnaiset UX-toiminnot (esim. asetusten muistaminen).",
    ...
    et: {
      ...
      analytics_desc:
        "Anonüümne kasutusanalüütika (GA4) teenuse parandamiseks.",
      marketing_desc: "Konversioonide jälgimine reklaamplatvormidel.",
      functional_desc:
        "Valikulised UX-funktsioonid (nt eelistuste mäletamine).",
  ...
  function textsFor(profile) {
    // Prefer the language the user explicitly chose via the site's own
    // language switcher over the geo-IP-derived profile language, so the
    // cookie banner matches whatever locale they're actually reading in.
    var siteLang = null;
    try {
      siteLang = localStorage.getItem("advocat_lang");
    } catch (_) {}
    if (siteLang && TEXTS[siteLang]) return TEXTS[siteLang];
  ...
  function readV2() {
    try {
      ...
      // Sanity check shape.
      if (!parsed.purposes || typeof parsed.purposes.analytics !== "boolean")
        return null;
      return parsed;
    } catch (e) {
      return null;
  ...
  function emitConsent(record) {
    var anyGranted = !!(
      record.purposes.analytics ||
      record.purposes.marketing ||
      record.purposes.functional
    );
    try {
      window.dispatchEvent(
        new CustomEvent("advocat:cookie-consent", {
          detail: {
            accepted: anyGranted,
    ...
    ["analytics", "marketing", "functional"].forEach(function (purpose) {
      try {
        window.dispatchEvent(
          new CustomEvent("advocat:consent:" + purpose, {
  ...
  function injectStyles() {
    ...
    var css =
      ...
      "  background: #0F1B35; color: #fff; border: 1px solid rgba(20,184,166,0.35);" +
      ...
      "#" +
      CONTAINER_ID +
      " a { color: #14B8A6; text-decoration: underline; }" +
      "#" +
      CONTAINER_ID +
      " .advCcRow { display: flex; gap: 10px; flex-wrap: wrap; align-items: stretch; }" +
      ...
      "#" +
      CONTAINER_ID +
      " .advCcBtn { flex: 1 1 auto; min-width: 140px; min-height: 44px;" +
      "  padding: 10px 14px; border: 0; border-radius: 8px; font-weight: 600; font-size: 14px;" +
      "  cursor: pointer; }" +
      "#" +
      CONTAINER_ID +
      " .advCcAccept { background: #14B8A6; color: #fff; }" +
      "#" +
      CONTAINER_ID +
      " .advCcReject { background: #475569; color: #fff; }" +
      "#" +
      CONTAINER_ID +
      " .advCcManage { background: transparent; color: #14B8A6;" +
      "  border: 1px solid rgba(20,184,166,0.45); }" +
      "#" +
      CONTAINER_ID +
      " .advCcAuthority { font-size: 11px; color: #94a3b8; margin-top: 10px; }" +
      "#" +
      CONTAINER_ID +
      " .advCcPurposes { margin: 12px 0; }" +
      "#" +
      CONTAINER_ID +
      " .advCcPurpose { display: flex; gap: 12px; padding: 10px 0;" +
      "  border-top: 1px solid rgba(255,255,255,0.08); }" +
      "#" +
      CONTAINER_ID +
      " .advCcPurpose:first-child { border-top: 0; }" +
      "#" +
      CONTAINER_ID +
      " .advCcPurpose label { font-weight: 600; }" +
      "#" +
      CONTAINER_ID +
      " .advCcPurpose small { color: #94a3b8; display: block; margin-top: 2px; font-weight: normal; }" +
      "#" +
      CONTAINER_ID +
      " .advCcSwitch { flex: 0 0 auto; align-self: center; }" +
      "#" +
      CONTAINER_ID +
      " .advCcSwitch input { width: 18px; height: 18px; }" +
      "#" +
      CONTAINER_ID +
      " .advCcSwitch input:disabled { opacity: 0.5; }" +
      "#" +
      CONTAINER_ID +
      ".minimal p { margin-bottom: 8px; }" +
      ...
      "  #" +
      CONTAINER_ID +
      " { left: 8px; right: 8px; bottom: 8px; padding: 14px; }" +
      "  #" +
      CONTAINER_ID +
      " .advCcBtn { width: 100%; flex: 1 1 100%; }" +
      "}";
  ...
  function hideLegacyBanner() {
    try {
      document.body.classList.add("advCcV2Active");
    } catch (_) {}
  }
  ...
  function renderLayer1(profile, t) {
    ...
    el.innerHTML =
      ...
      "</p>" +
      '<div class="advCcRow">' +
      '<button type="button" class="advCcBtn advCcAccept" data-action="accept_all">' +
      esc(t.accept_all) +
      "</button>" +
      '<button type="button" class="advCcBtn advCcReject" data-action="reject_all">' +
      esc(t.reject_all) +
      "</button>" +
      '<button type="button" class="advCcBtn advCcManage" data-action="manage">' +
      esc(t.manage) +
      "</button>" +
      "</div>" +
      '<div class="advCcAuthority">' +
      esc(t.authority_label) +
      ": " +
      esc(profile.authority) +
      "</div>";

    // Wire up clicks.
    Array.prototype.forEach.call(
      el.querySelectorAll("button[data-action]"),
      function (b) {
        b.addEventListener("click", function (ev) {
          ev.preventDefault();
          var a = b.getAttribute("data-action");
          if (a === "accept_all")
            return recordChoice(profile, "accept_all", {
              necessary: true,
              analytics: true,
              marketing: true,
              functional: true,
          ...
          if (a === "reject_all")
            return recordChoice(profile, "reject_all", {
              necessary: true,
              analytics: false,
              marketing: false,
              functional: false,
  ...
  function renderLayer2(profile, t) {
    ...
    el.innerHTML =
      ...
      esc(t.manage) +
      "</h3>" +
      '<div class="advCcPurposes" role="group" aria-label="' +
      esc(t.manage) +
      '">' +
      purposeRow(
        "necessary",
      ...
      "</div>" +
      '<div class="advCcRow">' +
      '<button type="button" class="advCcBtn advCcAccept" data-action="save">' +
      esc(t.save) +
      "</button>" +
      '<button type="button" class="advCcBtn advCcReject" data-action="reject_all">' +
      esc(t.reject_all) +
      "</button>" +
      '<button type="button" class="advCcBtn advCcManage" data-action="back">' +
      esc(t.back) +
      "</button>" +
      "</div>" +
      '<div class="advCcAuthority">' +
        ...
        b.addEventListener("click", function (ev) {
          ...
          if (a === "save") {
            var purposes = {
              necessary: true,
              analytics:
                !!document.getElementById("advCcPurpose_analytics") &&
                document.getElementById("advCcPurpose_analytics").checked,
              marketing:
                !!document.getElementById("advCcPurpose_marketing") &&
                document.getElementById("advCcPurpose_marketing").checked,
              functional:
                !!document.getElementById("advCcPurpose_functional") &&
                document.getElementById("advCcPurpose_functional").checked,
            };
            var anyOptional =
              purposes.analytics || purposes.marketing || purposes.functional;
  ...
  function recordChoice(profile, choice, purposes) {
    ...
    var record = {
      ...
      ts: now,
      country:
        window.advocatGeo && window.advocatGeo.country
          ? window.advocatGeo.country
          : null,
